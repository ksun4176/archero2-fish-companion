...
export default function Home() {
  ...
  const [fishSet, setFishSet] = useState('1');
  const lastUpdatedFishSetTime = useMemo(() => new Date(2025, 8, 13), []);
  ...
  const [lakesData, setLakesData] = useState<{ [lake in Lake]: LakeData }>({
    [Lake.LAKEHOUSE]: {
      fishCounts: getFishCounts(Lake.LAKEHOUSE),
      actionStack: [],
    },
    [Lake.SUNSET]: { fishCounts: getFishCounts(Lake.SUNSET), actionStack: [] },
    [Lake.OCEANIC]: {
      fishCounts: getFishCounts(Lake.OCEANIC),
      actionStack: [],
    },
  ...
  useEffect(() => {
    ...
    const savedFishSet = localStorage.getItem('fishingTrackerFishSet');
    if (savedFishSet) {
      try {
        const data = JSON.parse(savedFishSet) as FishSetStoredState;
        let canSaveFishSet = true;
        if (data.fishSetTime) {
          canSaveFishSet =
            new Date(data.fishSetTime).getTime() >
            lastUpdatedFishSetTime.getTime();
        }
        if (data.fishSet && canSaveFishSet) {
          setFishSet(data.fishSet);
        }
      } catch {
        console.warn('Failed to load saved fish set');
      }
    }
    setLoading(false);
  }, [lastUpdatedFishSetTime]);
  ...
  useEffect(() => {
    const state: FishSetStoredState = {
      fishSet,
      fishSetTime: new Date(),
    };
    localStorage.setItem('fishingTrackerFishSet', JSON.stringify(state));
  }, [fishSet]);
  ...
  const getTotalCaughtInfo = () => {
    const totalCaught = Object.keys(totalData).map((key) => {
      ...
      return {
        lake: lake,
        amount: Object.values(totalData[lake].fishCounts).reduce(
  ...
  const getOddsInfo = (lake: Lake) => {
    ...
    const percentage =
      numUncaughtFish > 0
        ? fishCounts[FishType.LEGENDARY] / numUncaughtFish
        : 0;
    let color = '';
    let symbol = '';
  ...
  const { fishCounts: currentFishCounts, actionStack: currentActionStack } =
    currentLakesData;
  ...
  const addWholePoolToTotalCount = (undo?: boolean) => {
    ...
    Object.keys(fishCounts).forEach((key) => {
      ...
      fishCounts[fishType] =
        totalData[currentLake].fishCounts[fishType] +
        fishCounts[fishType] * (undo ? -1 : 1);
    });
    setTotalData((prev) => ({
      ...prev,
      [currentLake]: {
  ...
  const resetAll = () => {
    ...
    const resetTotalData: Record<Lake, TotalData> = {} as Record<
      Lake,
      TotalData
    >;
    Object.values(Lake).forEach((lake) => {
  ...
  if (loading) {
    return (
      <div className='max-w-md mx-auto p-4 min-h-screen flex flex-col items-center justify-center'>
    ...
    <div className='max-w-md mx-auto p-4 min-h-screen'>
      ...
      {showInfo && (
        <div className='mb-4 p-2 bg-secondary border rounded-lg text-sm'>
          ...
          <ul className='space-y-1 list-disc pl-4'>
            ...
            <li>
              If you have enough silver tickets, go for easiest gold tickets
            </li>
          </ul>
        </div>
      )}
      ...
      <div className='flex flex-col items-center mb-4'>
        <div className='w-full relative flex items-center justify-center mb-2 h-8'>
          ...
          <Select
            value={fishSet}
            onValueChange={(newVal) => setFishSet(newVal)}
          >
            <SelectTrigger
              size='sm'
              className='w-[80px] absolute right-0'
            >
              <SelectValue placeholder='Select a Fish Set' />
            </SelectTrigger>
            <SelectContent>
              <SelectGroup>
            ...
            return (
              ...
              >
                <div className='flex flex-col'>
                  <div>{lake}</div>
                  <div className='text-xs'>{`${getNumUncaughtFish(
                    lake
                  )} fish`}</div>
        ...
        <div className='grid grid-cols-3 gap-2'>
          {Object.values(FishType).map((type) => (
            <Button
              ...
              className={`h-fit ${getFishBgColor(
                type
              )} text-shadow-black text-shadow-sm/50 dark:text-shadow-none`}
              aria-label={`Catch ${type.toLowerCase()} fish`}
            >
              <div className='flex flex-col'>
                <img
                  className='h-9'
                  src={`${getFishImgSrc(fishSet, currentLake, type)}`}
